...
class Board {
    ...
    /** Returns spot at Column C and Row R. */
    ...
    boolean isLegal(Move move) {
        ...
        if (get(move.getCol0(), move.getRow0()).side()
                != (turn())) {
            return false;
        }
        if (get(move.getCol1(), move.getRow1())
                == (get(move.getCol0(), move.getRow0()))) {
            return false;
        }
        ...
        while (col0 != col1 | row0 != row1) {
            if (boardspots[col0][row0].getPiece().side()
                    ==  pieceSide.opponent()) {
                return false;
            }
            col0 += Math.signum(col1 - col0);
    ...
    int piecesOnLine(Move move) {
        ...
        while ((col0 > 1 & col0 < 8 | colincr == 0)
                & ((row0 > 1 & row0 < 8) | rowincr == 0)) {
            col0 -= colincr;
            row0 -= rowincr;
        }
        int numPieces = 0;
    ...
    /** Returns number of pieces along line of move from SPOT in
     * X Y direction. */
    int piecesOnLine(BoardSpot spot, int x, int y) {
        ...
        double colincr = Math.signum(col1 - col0);
        double rowincr = Math.signum(row1 - row0);
        while (((colincr == 0) | col0 > 1 & col0 < 8) & ((rowincr == 0)
                | (row0 > 1 & row0 < 8))) {
            col0 -= colincr;
            row0 -= rowincr;
        }
    ...
    /** Returns legal Move from SPOT in direction X Y. */
    ...
    /** Returns all legal moves from SPOT. */
    ...
    /**Sets side to SIDE0. */
    protected void setSide(Side side0) {
        side = side0;
    }

    /** Represents board configuration. */
    private Piece[][] setup = new Piece[8][8];
    /** Array of spots on the board. */
    private BoardSpot[][] boardspots = new BoardSpot[10][10];


    /**
     * @return the boardspots
     */
    ...
    /**
     * @param boardspots0 the boardspots to set
     */
    public void setBoardspots(BoardSpot[][] boardspots0) {
        boardspots = boardspots0;
    }
    ...
    /**
     * @param numMoves0 the numMoves to set
     */
    public void setNumMoves(int numMoves0) {
        numMoves = numMoves0;
    }

    /** Stack of pieces that were taken. */
    ...
    /** Sets moveesMade to MOVESMADE0. */
    public void setMovesMade(LinkedList<Move> movesMade0) {
        movesMade = movesMade0;
    }
    /** Stack of moves made. */
